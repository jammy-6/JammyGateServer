...
bool MysqlDao::checkUserExist(const std::string &name,
  ...
  try {
    ...
    if (res->next()) {
      ...
      if (count != 0) {
        spdlog::info("MySQL查询结果：用户{}或邮箱{}已存在", name.c_str(),
                     email.c_str());
        pool_->returnConnection(std::move(con));
        return false;
      }
    ...
    pool_->returnConnection(std::move(con));
    spdlog::error("MySQL服务调用异常：信息为 {} ,错误码%d,SQLState为", e.what(),
                  e.getErrorCode(), e.getSQLState().c_str());
    return -1;
  }
}
...
bool MysqlDao::checkUserPassword(const std::string &name,
  ...
  try {
    ...
    if (res->next()) {
      ...
      userInfo.pwd = passwd;
      spdlog::info(
          "MySQL调用，获取用户信息,UID：%d，用户名：{}，邮箱：{}，密码为%成功",
          userInfo.uid, userInfo.name.c_str(), userInfo.email.c_str(),
          userInfo.pwd.c_str());
      pool_->returnConnection(std::move(con));
      return true;
    } else {
      spdlog::info("MySQL调用，用户登录验证密码失败，用户名：{}，密码：{}",
