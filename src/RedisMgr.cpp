...
RedisMgr::RedisMgr() {
	...
	spdlog::info("RedisMgr初始化成功,地址为 {}:{:d}，认证密码为{}",
				 redisIP.c_str(), port, password.c_str());
	_conn_pool.reset(
...
bool RedisMgr::set(const std::string &key, const std::string &value) {
	...
	// 如果返回NULL则说明执行失败
	if (NULL == reply) {
		spdlog::info("Execut command SET [key = {},value={}] failed ",
					 key.c_str(), value.c_str());
		freeReplyObject(reply);
...
bool RedisMgr::hSet(const char *key, const char *hkey, const char *hvalue,
	...
	if (reply == nullptr || reply->type != REDIS_REPLY_INTEGER) {
		spdlog::info(
			"Execut command HSet [key = {},hkey = {},hvalue = {}] failed ", key,
			hkey, hvalue);
		freeReplyObject(reply);
		_conn_pool->returnConnection(connection);
		return false;
